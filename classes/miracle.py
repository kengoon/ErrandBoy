...
class TopLayer(MDBoxLayout):
    pass


class FloatingButton(BoxLayout):
    pass
...
class ScrollLayer(ThemableBehavior, StencilView, RelativeLayout):
    ...
    def add_widget(self, widget, index=0, canvas=None):
        if issubclass(widget.__class__, LayerContent):
            widget._root = self
            return self.ids.scrollbox.add_widget(widget)

        elif issubclass(widget.__class__, TopLayer):
            return self.ids._toolbar.add_widget(widget)

        elif issubclass(widget.__class__, MiddleLayer):
            return self.ids._pin_widget.add_widget(widget)

        elif issubclass(widget.__class__, FloatingButton):
    ...
    def _update_floating_button_pos(self, *args):
        ...
        if self.float_button_pos in ["bl", "tr", "tl"]:
            raise NotImplementedError

        elif self.float_button_pos == "br":
